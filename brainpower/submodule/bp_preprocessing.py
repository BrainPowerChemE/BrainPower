...
def features_ina_string(frame, uniqueness):
    ...
    def unique_features_to_string(X,uniqueness):
        ...
        if type(uniqueness) == int:
            ...
            raise Exception( "Invalid value for parameter 'uniqueness'. Use 'all' or an integer.")
        hyphen_items = []
...
def altair_feature_selection_chart(frame, uniqueness):
    ...
    selection = alt.selection_single(empty='none',nearest=True)
    ...
    df_exploded.head(10)    
    error_bars = alt.Chart(df_exploded).mark_boxplot().encode(
    ...
    df['featuresX'] = df['uniqueX_features_ina_string'].astype(str).str.split('-') # Create a list to split into multiple lines in the chart
    df['zero'] = 0
    text = alt.Chart(df).mark_text(baseline='bottom', dy=-75
    ).encode(
        y='zero',
        text="featuresX"
    ).transform_filter(
